...
echo "$($PSQL "SELECT name FROM (SELECT name FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE year = 2014 UNION SELECT name FROM games INNER JOIN teams ON games.opponent_id = teams.team_id WHERE year = 2014) AS temp ORDER BY name")"
